...
export default function Predict() {
  ...
  useEffect(() => {
    getTeams().then(res => {
      setTeams(res.data)
      if (res.data.length > 0) setHome(res.data[0])
      if (res.data.length > 1) setAway(res.data[1])
    })
  }, [])

  const handlePredict = () => {
    if (!home || !away || home === away) return alert("Pick two different teams")
    setLoading(true)
  ...
  return (
    <div style={{ padding: "32px", maxWidth: "600px", margin: "0 auto" }}>
      <h1 style={{ marginBottom: "8px" }}>Match predictor</h1>
          ...
          {[
            ...
            <div key={label} style={{ marginBottom: "12px" }}>
              ...
              <div style={{ background: "#f0f0f0", borderRadius: "4px", height: "8px" }}>
                <div style={{ background: color, height: "8px", borderRadius: "4px",
